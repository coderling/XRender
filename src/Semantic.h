...
namespace XRender
{
    ...
    #define BIND_VERTEXINPUT_SEMANTIC(semantic, T)\
    this->vertex_intput_semantic |= static_cast<uint32_t>(semantic);

    #define BIND_VERTEXOUTPUT_SEMANTIC(semantic, T)\
    {\
    this->vertex_output_semantic |= static_cast<uint32_t>(semantic);\
    ...
    [](VertexOutput& out, VertexOutput* triangle[], const SEMANTIC& tsemantic, const Vec3f& barycentric)\
        {\
            ...
            T property3; GET_DATA_BY_SEMATIC(property3, (*(triangle[3])), tsemantic, T);\
